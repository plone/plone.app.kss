...
class TestContentsTabs:
    ...
    def test_ul_id():
        r"""
        We actually authenticate and we'll doing some tests about ul id
        (which it should be present)
        ...
        a tags inside of the li tags shouldn't have ids; li tags should have id
        attributes. There may be three or four tags (contentview-history was
        removed in Plone 3.3).
...
class TestContentMenu:
    ...
    def test_menu_presence():
        r"""
        We must authenticate because content menu is only present for
        logged in users.
        ...
        We are in a page so we must have the "change workflow" menu.
            ...
            >>> contentmenu_dl_tag = soup.find('dl',
        ...
        Then you must have the copy and cut links
            ...
            >>> soup.find('a', {'class':'actionicon-object_buttons-cut'}) \
            ... is not None
            True

            >>> soup.find('a', {'class':'actionicon-object_buttons-copy'}) \
            ... is not None
            True
        ...
        A couple of things again, basically the stuff we hook on with kss,
        which are, the external div and the inner ul (one is used for content
        replacement through innerHTML, the other used to hook events on load)
        ...
        Ok let's see if we have the hook to replace the content
            ...
            >>> content_div = soup.find('div', {'id': 'portal-column-content'})
            >>> content_div is not None
            True
            >>> content_actions = content_div.find('div',
            ... {'class': 'contentActions'})
            >>> content_actions is not None
            True

        This is great, now let's check if we have the bind load stuff in place
        too.
